// app/(auth)/forgot-password/page.tsx - STATIC VERSION
'use client';
...
export default function ForgotPasswordPage() {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!cardRef.current || status !== 'input') return;
  ...
  // LOGIC FORGOT PASSWORD LANGSUNG DI SINI
  const handlePasswordReset = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!identifier) {
        setError('Email/WhatsApp wajib diisi');
        setStatus('input');
        return;
      }
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Gagal mengirim instruksi');
        setStatus('input');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center font-sans px-4 py-12 overflow-hidden">
      <div className="grid grid-cols-1 md:grid-cols-2 max-w-5xl w-full rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        {/* Kolom Kanan: Forgot Password Form */}
        <motion.div
          ref={cardRef}
          onMouseMove={handleMouseMove}
          onMouseLeave={handleMouseLeave}
          style={{ transform: transformStyle }}
          whileHover={{ scale: status === 'input' ? SCALE_HOVER : 1 }}
